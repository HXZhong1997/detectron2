...
@ROI_HEADS_REGISTRY.register()
class StandardROIHeads_NetG(ROIHeads):
    """
    ...
    def forward(
        ...
        if self.training:
            losses = self._forward_box(features, proposals, netG, mask)
            # Usually the original proposals used by the box head are used by the mask, keypoint
            # heads. But when `self.train_on_pred_boxes is True`, proposals will contain boxes
            # predicted by the box head.
            losses.update(self._forward_mask(features, proposals,netG)) #TODO
            losses.update(self._forward_keypoint(features, proposals, netG)) #TODO
    ...
    def _forward_box(self, features: Dict[str, torch.Tensor], proposals: List[Instances], netG: nn.Module = None, mask: torch.Tensor=None):
        ...
        if self.training and netG is not None:
            ...
            box_features_g = self.box_head(box_features_g)
            predictions_g = self.box_predictor(box_features_g)
            del box_features_g
    ...
    def _forward_mask(self, features: Dict[str, torch.Tensor], instances: List[Instances], netG: nn.Module=None):
        """
    ...
    def _forward_keypoint(self, features: Dict[str, torch.Tensor], instances: List[Instances], netG: nn.Module = None):
        """
