...
def do_train(cfg_g, model_g, cfg_det, model_det, resume=False):
    ...
    optimizer = build_optimizer(cfg_g, model_g)
    scheduler = build_lr_scheduler(cfg_g, optimizer)
    ...
    start_iter = (
        checkpointer.resume_or_load(cfg_g.MODEL.WEIGHTS, resume=resume).get("iteration", -1) + 1
    )
    max_iter = cfg_g.SOLVER.MAX_ITER

    periodic_checkpointer = PeriodicCheckpointer(
        checkpointer, cfg_g.SOLVER.CHECKPOINT_PERIOD, max_iter=max_iter
    )

    writers = default_writers(cfg_g.OUTPUT_DIR, max_iter) if comm.is_main_process() else []
